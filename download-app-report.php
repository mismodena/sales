...
// ngisi nilai & styling bagian header tabel
	...
	$objPHPExcel->getActiveSheet()
			...
			->setCellValue("O5", "PPh21")
			->setCellValue("P5", "KOMISI FINAL")
			->setCellValue("Q5", "KOMISI YANG DIBAYARKAN");
			//->setCellValue("N5", "KETERANGAN");
	$objPHPExcel->getActiveSheet()->getStyle('A5:Q5')->applyFromArray($arr_font_style);		
	$objPHPExcel->getActiveSheet()->getStyle('A5:Q5')->applyFromArray($arr_bgcolor_style);
	$objPHPExcel->getActiveSheet()->getStyle('A5:Q5')->applyFromArray($arr_border_style);
	...
	while( $komisi = sqlsrv_fetch_array( $rs_komisi ) ){
		...
		$objPHPExcel->getActiveSheet()
			...
			->setCellValue("O" . ($end_nomor), $komisi["pph"] )
			->setCellValue("P" . ($end_nomor), $komisi["komisi_final"] )
			->setCellValue("Q" . ($end_nomor), ceil($komisi["komisi_final"] / 50) * 50 );
		...
		$objPHPExcel->getActiveSheet()->getStyle('P' . ($end_nomor) )->getNumberFormat()->setFormatCode('#,###');
		$objPHPExcel->getActiveSheet()->getStyle('Q' . ($end_nomor) )->getNumberFormat()->setFormatCode('#,###');
	...
	header('Content-type: application/vnd.ms-excel');
	header('Content-Disposition: attachment; filename="report-komisi-final-'. $_REQUEST["area"] .'-'. $formatted_periode .'.xls"');
	$lokasi_simpan_file = "php://output";
	$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
	$objWriter->save( $lokasi_simpan_file );
