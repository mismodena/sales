...
function tambah_penjualan(){
	...
	$('#table-data tr:nth-last-child(2) td:nth-child(12)').html( '<?=isian_sn()?>'.replace(/#counter#/gi, nomor) );
...
function klik_checbox(ob){
	...
	var bg = '#FFF';
	if( document.getElementById( 'cb_' + string_cbox_id(ob) ).checked )	bg = '#80ff80' ;
...
function __upload(ob){
	...
	f.appendChild(ob);
	f.action='?c=upload&id=' +ob.id
	f.target='frmx'
	document.body.appendChild(f)
	f.submit()
}

function cekNoSeri(ob){
	var i = ob.id.split(/_/gi);
	var sn = document.getElementById('sn_' + i[1]);
	var trans = document.getElementById('transaksi_' + i[1]);
	var id = trans.value.split("|");
				...
				if(xmlhttp.readyState==4){
					if(xmlhttp.status==200){
						if(xmlhttp.responseText=="0"){
							alert("Nomor seri yang dimasukkan salah!");
							//sn.focus();
						}else if(xmlhttp.responseText=="1"){
							alert("Nomor seri telah digunakan!");
							//sn.focus();
						}
					}
				}
			}		
			try{xmlhttp.open("GET",'cek_no_seri.php?sn='+sn.value+'&id_p='+id[0]+'&id_d='+id[1], true);
