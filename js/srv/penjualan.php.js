...
function ubah_qty_harga(ob){
	...
	var index_item = $.inArray(item.value, arr_daftar_item);
...
function itung_grand_total(){
	...
	while(true == true){
		try{
			...
			if( grand_total != parseFloat(String(document.getElementById('hd_realisasi_android').value).trim()) ){
...
function tambah_penjualan(){
	...
	var baris = '<tr>';
	for(var x=1; x<=6; x++)	baris += '<td></td>';
	baris += '</tr>';
	
	$('#table-data tr:last').before(baris);
	$('#table-data tr:nth-last-child(2) td:first-child').html(nomor);
	$('#table-data tr:nth-last-child(2) td:nth-child(2)').html( '<?=pilihan_item("#counter#", bikin_opsi_selectbox( $arr_opsi_selectbox_item ) )?>'.replace(/#counter#/gi, nomor) );
	$('#table-data tr:nth-last-child(2) td:nth-child(3)').html( '<span id="scampaign_#counter#"></span>'.replace(/#counter#/gi, nomor) );
	$('#table-data tr:nth-last-child(2) td:nth-child(4)').html( '<?=isian_harga()?>'.replace(/#counter#/gi, nomor) );
	$('#table-data tr:nth-last-child(2) td:nth-child(5)').html( '<?=isian_qty()?>'.replace(/#counter#/gi, nomor) );
	$('#table-data tr:nth-last-child(2) td:nth-child(6)').html( '<?=isian_hidden()?>'.replace(/#counter#/gi, nomor) );
...
function kembali(){
	if( document.getElementById('perubahan_data').value == "1" ){
		...
		location.href='index.php?area=<?=$_REQUEST["area"]?>&periodeid=<?=$_REQUEST["periodeid"]?>'
}

function ubah_tampilan(){
	if( document.getElementById('perubahan_data').value == "1" ){
		if( confirm('Terdapat perubahan data penjualan!\nApakah akan disimpan terlebih dahulu dan kembali ke halaman sebelumnya ?') )
			__submit('', 'c=simpan_data&ubah_tampilan=<?=$mode_tampilan["lain"]?>');
	}else
		location.href='penjualan.php?v=<?=$mode_tampilan["lain"]?>&user_id=<?=$_REQUEST["user_id"]?>&periodeid=<?=$_REQUEST["periodeid"]?>&area=<?=$_REQUEST["area"]?>'
}
...
function _reset(){
	if( confirm('Batalkan semua perubahan yang belum disimpan ?') )
		location.href='penjualan.php?user_id=<?=$_REQUEST["user_id"]?>&periodeid=<?=$_REQUEST["periodeid"]?>&area=<?=$_REQUEST["area"]?>'
}
