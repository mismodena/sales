...
if( $_REQUEST["c"] =="simpan_data" ){	
	...
	if( isset($_REQUEST["kembali"]) ) header("'location:verifikasi-penjualan.php?area=". $_REQUEST["area"] ."&tanggal_awal=". $_REQUEST["tanggal_awal"] ."&tanggal_akhir=". $_REQUEST["tanggal_akhir"]);
	
	header("location:verifikasi-penjualan-detail.php?area=". $_REQUEST["area"] ."&user_id=". $_REQUEST["user_id"] ."&tanggal_awal=". $_REQUEST["tanggal_awal"] ."&tanggal_akhir=". $_REQUEST["tanggal_akhir"]);
}
...
while( $daftar_item = sqlsrv_fetch_array( $rs_daftar_item ) ){
	...
	$arr_harga_item[] =$daftar_item["unitprice"] * (100 - $data_penjualan["level_diskon"]) / 100 ;
	...
	$arr_item_info[] = array(
						...
						"harga" =>$daftar_item["unitprice"] * (100 - $data_penjualan["level_diskon"]) / 100,
