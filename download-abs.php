...
function download(){
	...
	jalankan_proses_di_iframe('download-abs.php?c='+mode+'&area=<?=$cabang?>&periodeid=<?=@$_REQUEST["periodeid"]?>');
}
</script>
<style>
...
<div style="background-image:url('images/18-512.png'); background-repeat:no-repeat; background-position:center; height:97%;">
	...
	<ul style="margin-top:-47px; margin-left:-27px">
		<li><input type="radio" name="cb" id="cb1" value="report" checked><label for="cb1">Report komisi Poin</label><br /><div style="padding-left:27px">Download reporting excel berisi baris data MM dengan informasi jam kerja, hari kerja, training dan komponen poin beserta hasil komisi poin.</div></li>
	<? if(is_null($_SESSION["area"])){ ?>
		<li><input type="radio" name="cb" id="cb2" value="detail"><label for="cb2">Report Insentif dibayarkan</label><br /><div  style="padding-left:27px">Download report hasil akhir komisi mitra modena.</div></li>
	<?}else{?>
		<input type="hidden" name="cb" id="cb2" value="detail" />
	<?}?>
	</ul>
	</p>
