...
// LOAD DATA KOMISI
$rs_komisi = komisi::load_data_komisi_final( 
					array(
...
while( $data_komisi = sqlsrv_fetch_array($rs_komisi) ){
	$arr_data_komisi = array(
		...
		strtoupper($data_komisi["bank"]),
		strtoupper($data_komisi["rekening"]),
...
$sql = "select * from  stat_periode_header where periode=".$periodeid_terbaru." and area ='".$default_area."' and status = 1";
$rs_ = sql::execute($sql);
if( sqlsrv_num_rows( $rs_ ) == 0 )
	@$script .= "
			document.getElementById('btnDown').innerHTML = '<img src=\"images/excel.png\" style=\"border:none; width:137px; padding:0px 77px 0px 77px\" /><br />Belum Verifikasi';
		";
